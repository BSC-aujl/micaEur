...
describe('MiCA EUR Program Tests (Deployed Version)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (program.programId.toString() !== PROGRAM_ID.toString()) {
        console.warn('Warning: Program ID mismatch. Using:', program.programId.toString(), 'Expected:', PROGRAM_ID.toString());
      }
  ...
  describe('KYC Oracle', () => {
    it('should have the correct account structure', async () => {
      try {
        console.log(`Checking KYC Oracle structure at ${kycOracleStatePda.toString()}`);
  ...
  async function fundTestAccounts() {
    try {
      ...
      // Log funding success
      console.log(`Funded payer ${payer.publicKey.toString().slice(0, 8)}... with 2 SOL`);
