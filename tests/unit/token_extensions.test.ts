...
async function getGroupPolicyAuthorityAddress(
  program: anchor.Program,
  groupPDA: PublicKey
): Promise<[PublicKey, number]> {
  const groupAccount = await program.account.group.fetch(groupPDA);
  if (!groupAccount.policyAuthority) {
    throw new Error("Policy authority is null");
  }
  if (!groupAccount.policyAuthorityBump) {
    throw new Error("Policy authority bump is null");
  }
  return [groupAccount.policyAuthority, groupAccount.policyAuthorityBump];
}

async function getGroupPolicyAuthorityAddressCheck(
  program: anchor.Program,
  groupPDA: PublicKey
): Promise<[PublicKey, number]> {
  const groupAccount = await program.account.group.fetch(groupPDA);
  ...
  return [groupAccount.policyAuthority, groupAccount.policyAuthorityBump || 0];
}
