...
describe('Blacklist Functionality Tests', () => {
  ...
  before(async () => {
    ...
    normalUserTokenAccount = new PublicKey("NormalUserTokenAccXXXXXXXXXXXXXXXXXXXXXXXXX");
    blacklistedUserTokenAccount = new PublicKey("BlacklistedUserTokenAccXXXXXXXXXXXXXXXXXX");
    temporarilyBlacklistedUserTokenAccount = new PublicKey("TempBlacklistedTokenAccXXXXXXXXXXXXXXXXXX");
  });
  ...
  describe('Blacklist Management', () => {
    it('should be able to add a user to the blacklist', async () => {
      ...
      const isUserBlacklisted = await isBlacklisted(context, blacklistedUser.publicKey);
    ...
    it('should be able to update a blacklist entry', async () => {
      ...
      const status = await getBlacklistStatus(context, blacklistedUser.publicKey);
      
      // Verify the data was updated
      assert.equal(status.reason, BlacklistReason.KycRevoked, 'Reason should be updated');
      assert.equal(status.evidence, 'KYC verification revoked due to suspicious documentation', 'Evidence should be updated');
    });
    ...
    it('should be able to add a user to the temporary blacklist', async () => {
      ...
      // Get blacklist status
      const status = await getBlacklistStatus(context, temporarilyBlacklistedUser.publicKey);
      ...
      assert.isAtMost(status.remainingTime!, 3600, 'Remaining time should be 3600 seconds or less');
    });
    ...
    it('should be able to remove a user from the blacklist', async () => {
      ...
      const isUserBlacklisted = await isBlacklisted(context, temporarilyBlacklistedUser.publicKey);
      assert.isFalse(isUserBlacklisted, 'User should not be blacklisted anymore');
    });
  });
  ...
  describe('Blacklist Enforcement', () => {
    it('should prevent blacklisted users from transferring tokens', async () => {
      ...
      const blacklistStatus = await getBlacklistStatus(context, blacklistedUser.publicKey);
      assert.isTrue(blacklistStatus.isBlacklisted, 'User should still be blacklisted');
      ...
      assert.isTrue(blacklistStatus.isBlacklisted, 'Blacklisted user should have transfer restrictions');
    });
    
    it('should allow non-blacklisted users to transfer tokens', async () => {
      // Verify the normal user is not blacklisted
      const isUserBlacklisted = await isBlacklisted(context, normalUser.publicKey);
      assert.isFalse(isUserBlacklisted, 'Normal user should not be blacklisted');
      
      // In a real test, we would attempt a transfer and expect it to succeed
      // Since we're mocking, we'll just assert the correct state
      const blacklistStatus = await getBlacklistStatus(context, normalUser.publicKey);
      assert.isFalse(blacklistStatus.isBlacklisted, 'Normal user should not have blacklist restrictions');
    });
  });
}); 