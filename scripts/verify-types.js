...
function verifyRustTypes(filePath, content) {
  ...
  const publicFunctionMatches = [...content.matchAll(RUST_TYPE_PATTERNS.publicFunctionReturnType)];
  for (const match of publicFunctionMatches) {
    const returnType = match[1];
    if (!returnType || returnType.trim() === '') {
      issues.push(`Function at position ${match.index} is missing a proper return type`);
    }
  }
  
  // Check function parameters
  const functionParamMatches = [...content.matchAll(RUST_TYPE_PATTERNS.functionParams)];
  for (const match of functionParamMatches) {
    const params = match[1];
    if (params && !params.trim().includes(':')) {
      issues.push(`Function parameters at position ${match.index} are missing type annotations`);
    }
  }
  
  // Check account structs
  const accountStructMatches = [...content.matchAll(RUST_TYPE_PATTERNS.accountStructs)];
  if (accountStructMatches.length > 0) {
    // Check that we have appropriate derive macros
    const deriveMacroMatches = [...content.matchAll(RUST_TYPE_PATTERNS.deriveMacros)];
...
function verifyTsTypes(filePath, content) {
  ...
  const functionParamMatches = [...content.matchAll(TS_TYPE_PATTERNS.functionParams)];
  for (const match of functionParamMatches) {
    const params = match[1];
...
function verifyFileTypes(filePath) {
  try {
    if (!fs.existsSync(filePath)) {
      return { valid: false, issues: [`File does not exist: ${filePath}`] };
    }
