...
// Files to check
const filesToCheck = ["tests/kyc-end-to-end-flow.ts", "docs/kyc-flow-guide.md"];
...
for (const relativeFilePath of filesToCheck) {
  ...
  if (filePath.endsWith(".ts")) {
    ...
    const verificationLevelMatch = content.match(
      /VERIFICATION_LEVELS\s*=\s*{[^}]+}/s
    );
    ...
    if (verificationLevelMatch) {
      ...
      for (const [level, value] of Object.entries(
        ...
        if (!match) {
          console.error(
            `❌ Missing verification level "${level}" in ${relativeFilePath}`
          );
          hasErrors = true;
        } else if (parseInt(match[1], 10) !== value) {
          console.error(
            `❌ Verification level "${level}" has incorrect value in ${relativeFilePath}. Expected ${value}, got ${match[1]}`
          );
          hasErrors = true;
        }
      }
      ...
      console.warn(
        `⚠️ Could not find VERIFICATION_LEVELS constant in ${relativeFilePath}`
      );
    }
    ...
    if (content.includes("Unverified")) {
      const canTransfer =
        content.includes("Unverified User can transfer") ||
        content.includes("unverified ones, can transfer");
      const cannotMint =
        content.includes("Unverified User lacks") ||
      ...
      if (!canTransfer) {
        console.warn(
          `⚠️ Could not confirm Unverified users can transfer in ${relativeFilePath}`
        );
      }

      if (!cannotMint) {
        console.warn(
          `⚠️ Could not confirm Unverified users cannot mint in ${relativeFilePath}`
        );
      }
    }
  }
  ...
  if (filePath.endsWith(".md")) {
    ...
    const hasUnverified =
      ...
      !content.includes("Unverified.*mint");
    const hasBasic =
      content.includes("Basic Verification") &&
      content.includes("Individual") &&
      content.includes("mint") &&
      content.includes("redeem");
    const hasStandard =
      content.includes("Standard Verification") &&
      content.includes("Business") &&
      content.includes("higher limits");
    const hasAdvanced =
      content.includes("Advanced Verification") &&
      content.includes("Institutional");

    if (!hasUnverified) {
      console.error(
        `❌ Missing or incorrect Unverified level description in ${relativeFilePath}`
      );
      hasErrors = true;
    }
    ...
    if (!hasBasic) {
      console.error(
        `❌ Missing or incorrect Basic level description in ${relativeFilePath}`
      );
      hasErrors = true;
    }
    ...
    if (!hasStandard) {
      console.error(
        `❌ Missing or incorrect Standard level description in ${relativeFilePath}`
      );
      hasErrors = true;
    }
    ...
    if (!hasAdvanced) {
      console.error(
        `❌ Missing or incorrect Advanced level description in ${relativeFilePath}`
      );
      hasErrors = true;
    }
    ...
    if (hasTable) {
      ...
      for (const [level, operations] of Object.entries(EXPECTED_OPERATIONS)) {
        for (const operation of ["Transfer", "Mint", "Redeem"]) {
          ...
          if (!match) {
            console.error(
              `❌ Missing ${operation} operation row in permission table in ${relativeFilePath}`
            );
            hasErrors = true;
            continue;
          }
          ...
          const levelIndex = levelNames.findIndex(
          ...
          if (!hasExpectedMarker) {
            console.error(
              `❌ Incorrect permission for ${level} level and ${operation} operation in ${relativeFilePath}`
            );
            console.error(
              `   Expected: ${expectedMarker}, Found: ${actualMarker}`
            );
            hasErrors = true;
          }
        }
