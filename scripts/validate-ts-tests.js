...
if (process.env.SKIP_TS_TEST_VALIDATION === "true") {
  console.log(
    "ℹ️ Skipping TypeScript test validation as SKIP_TS_TEST_VALIDATION is set to true"
  );
  process.exit(0);
}
...
// Files allowed to skip chai/test utilities imports
const IMPORT_CHECK_ALLOWLIST = [
  "tests/new-approach/",
  "tests/e2e/",
  "tests/unit/",
  "tests/integration/",
...
const STRUCTURE_CHECK_ALLOWLIST = [
  "tests/new-approach/",
  "tests/e2e/",
  "tests/unit/",
  "tests/integration/",
...
function getTestFiles() {
  ...
  const unitFiles = fs
    .readdirSync(UNIT_TEST_DIR, { withFileTypes: true })
  ...
  const integrationFiles = fs
    .readdirSync(INTEGRATION_TEST_DIR, { withFileTypes: true })
    .filter((dirent) => dirent.isFile() && dirent.name.endsWith(".test.ts"))
  ...
  const e2eFiles = fs
    .readdirSync(E2E_TEST_DIR, { withFileTypes: true })
    .filter((dirent) => dirent.isFile() && dirent.name.endsWith(".test.ts"))
      ...
      const walkDir = (dir) => {
        fs.readdirSync(dir, { withFileTypes: true }).forEach((subDirent) => {
          ...
          if (subDirent.isDirectory()) {
            ...
            subDirent.isFile() &&
            subDirent.name.endsWith(".test.ts")
          ) {
            testFiles.push(res);
          }
        });
...
// Check if a file path is in an allowlist
function isInAllowlist(filePath, allowlist) {
  ...
  for (const allowedPath of allowlist) {
    // If allowedPath ends with a slash, treat it as a directory prefix
    if (allowedPath.endsWith("/") && normalizedPath.startsWith(allowedPath)) {
      return true;
    }
  }
...
function validateTestFile(filePath) {
  ...
  if (
    !isInAllowlist(filePath, IMPORT_CHECK_ALLOWLIST) &&
  ...
  if (
    !isInAllowlist(filePath, STRUCTURE_CHECK_ALLOWLIST) &&
  ...
  if (
    !isInAllowlist(filePath, ANY_TYPE_ALLOWLIST) &&
...
function main() {
  ...
  if (allValid) {
    log(
      "\n✅ All TypeScript tests validated successfully",
      COLOR.bright + COLOR.green
    );
  } else {
    log("\n❌ TypeScript test validation failed", COLOR.bright + COLOR.red);
    log(
      "Please fix the issues in your test files and try again.",
      COLOR.yellow
    );
  }
