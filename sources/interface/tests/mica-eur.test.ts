...
describe("MiCA EUR Program Tests (Deployed Version)", () => {
  ...
  it("should have a deployed program at the specified address", async () => {
    try {
      ...
      if (accountInfo) {
        ...
        console.log(
          "Program verified as executable at address:",
          PROGRAM_ID.toString()
        );
      } else {
        console.error(
          "Program account not found at address:",
          PROGRAM_ID.toString()
        );
      }
    } catch (err) {
  ...
  it("should have correct metadata in the program", async () => {
    try {
      ...
      console.log(
        "Program found on chain. For detailed verification, please use Solana CLI tools."
      );
    } catch (err) {
