...
describe("MiCA EUR Tests with LiteSVM", () => {
  ...
  describe("Token Functionality Tests", () => {
    test("should initialize mint info account", () => {
      // Setup the test environment
      const { svm, keypairs, pdas } = setupLiteSvmTestEnv(PROGRAM_ID);
      const { issuer, freezeAuthority, permanentDelegate, mintKeypair } =
        keypairs;
      ...
      expect(
        storedFreezeAuthority.equals(freezeAuthority.publicKey)
      ).toBeTruthy();
    ...
    test("should mint tokens to verified users", () => {
      // Setup the test environment
      const { svm, keypairs, pdas, tokenAccounts } =
        setupLiteSvmTestEnv(PROGRAM_ID);
      const {
        authority,
        issuer,
        freezeAuthority,
