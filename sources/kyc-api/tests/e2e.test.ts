...
describe("MiCA EUR KYC End-to-End Flow", () => {
  ...
  it("should demonstrate KYC flows with different verification levels", async () => {
    ...
    const individualUserKycData = Buffer.alloc(
      8 + 32 + 32 + 1 + 1 + 8 + 8 + 32 + 32 + 64
    );
    ...
    const businessUserKycData = Buffer.alloc(
      8 + 32 + 32 + 1 + 1 + 8 + 8 + 32 + 32 + 64
    );
    ...
    const mintInfoData = Buffer.alloc(
    ...
    const unverifiedUserTokenAccountInfo = svm.getAccount(
      unverifiedUserTokenAccount
    );
    const unverifiedUserTokenState = AccountLayout.decode(
      unverifiedUserTokenAccountInfo!.data
    );
    ...
    console.log(
      "6. Updating Individual User's KYC status to VERIFIED with BASIC level..."
    );
    ...
    Buffer.from(expiryTime.toString().padStart(8, "0")).copy(
      updatedIndividualUserKycData,
      82
    );
    ...
    expect(updatedIndividualUserAccount?.data[72]).to.equal(
      KYC_STATUS.VERIFIED
    );
    expect(updatedIndividualUserAccount?.data[73]).to.equal(
      VERIFICATION_LEVELS.BASIC
    );

    // STEP 7: Update Business User's KYC status to VERIFIED with STANDARD level
    console.log(
      "7. Updating Business User's KYC status to VERIFIED with STANDARD level..."
    );
    ...
    Buffer.from(expiryTime2.toString().padStart(8, "0")).copy(
      updatedBusinessUserKycData,
      82
    );
    ...
    expect(updatedBusinessUserAccount?.data[73]).to.equal(
    ...
    console.log(
      "9. Minting tokens to Individual User (has BASIC verification level)..."
    );
    ...
    const individualUserFinalAccount = svm.getAccount(
      individualUserTokenAccount
    );
    const individualUserFinalState = AccountLayout.decode(
      individualUserFinalAccount!.data
    );

    expect(individualUserFinalState.amount.toString()).to.equal(
      individualMintAmount.toString()
    );

    // STEP 10: Mint tokens to the Business User (with STANDARD verification level)
    console.log(
      "10. Minting tokens to Business User (has STANDARD verification level)..."
    );
    ...
    const businessUserFinalState = AccountLayout.decode(
      businessUserFinalAccount!.data
    );

    expect(businessUserFinalState.amount.toString()).to.equal(
      businessMintAmount.toString()
    );

    // STEP 11: Transfer tokens from Individual User to Unverified User (which is allowed)
    console.log(
      "11. Transferring tokens from Individual User to Unverified User..."
    );
    ...
    const individualUserAfterTransferAccount = svm.getAccount(
      individualUserTokenAccount
    );
    const unverifiedUserAfterTransferAccount = svm.getAccount(
      unverifiedUserTokenAccount
    );

    const individualUserAfterTransferState = AccountLayout.decode(
      individualUserAfterTransferAccount!.data
    );
    const unverifiedUserAfterTransferState = AccountLayout.decode(
      unverifiedUserAfterTransferAccount!.data
    );

    expect(individualUserAfterTransferState.amount.toString()).to.equal(
      (individualMintAmount - transferAmount).toString()
    );
    expect(unverifiedUserAfterTransferState.amount.toString()).to.equal(
      transferAmount.toString()
    );

    // STEP 12: Demonstrate that Unverified User can transfer tokens (to Business User)
    console.log(
      "12. Transferring tokens from Unverified User to Business User..."
    );
    ...
    // Verify the transfer was successful
    const unverifiedUserFinalAccount = svm.getAccount(
      unverifiedUserTokenAccount
    );
    const businessUserUpdatedAccount = svm.getAccount(businessUserTokenAccount);

    const unverifiedUserFinalState = AccountLayout.decode(
      unverifiedUserFinalAccount!.data
    );
    const businessUserUpdatedState = AccountLayout.decode(
      businessUserUpdatedAccount!.data
    );

    expect(unverifiedUserFinalState.amount.toString()).to.equal(
      (transferAmount - unverifiedTransferAmount).toString()
    );
    expect(businessUserUpdatedState.amount.toString()).to.equal(
      (businessMintAmount + unverifiedTransferAmount).toString()
    );

    // STEP 13: Simulate attempt to mint to Unverified User (which should fail)
    console.log(
      "13. Simulating attempt to mint to Unverified User (should fail)..."
    );
    console.log(
      "   This operation would fail because Unverified User lacks BASIC verification level"
    );

    // STEP 14: Demonstrate Individual User (BASIC) redeeming tokens
    console.log(
      "14. Simulating token redemption by Individual User (BASIC level)..."
    );
    ...
    const individualUserFinalRedemptionAccount = svm.getAccount(
      individualUserTokenAccount
    );
    const individualUserFinalRedemptionState = AccountLayout.decode(
      individualUserFinalRedemptionAccount!.data
    );
